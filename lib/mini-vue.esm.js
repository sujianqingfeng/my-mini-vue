    ...
    function setupRenderEffect(instance, container, anchor) {
        instance.update = effect(function () {
            if (!instance.isMounted) {
                ...
                console.log("update");
                var next = instance.next, vnode = instance.vnode, proxy = instance.proxy;
                if (next) {
                    next.el = vnode.el;
                    updateComponentPreRender(instance, next);
                }
                var prevTree = instance.subTree;
                var nextTree = instance.render.call(proxy, proxy);
...
function genElement(node, context) {
    ...
    var nullableNodes = genNullable([tag, props, children]);
...
function parseInterpolation(context) {
    ...
    var rowContentLength = closeIndex - openDelimiter.length;
    var rowContent = parseTextData(context, rowContentLength);
    var content = rowContent.trim();
...
function createRoot(children) {
    return { children: children, type: 4 /* ROOT */ };
}
/**
 * 判断文本开头是否是传入的tag
 *
...
function transformElement(node, context) {
    if (node.type === 2 /* ELEMENT */) {
        return function () {
            var children = node.children, tag = node.tag;
            var vnodeTag = "\"" + tag + "\"";
            var vnodeProps = undefined;
...
export { compileToFunction, createApp, createVNode as createElementVNode, createRenderer, createTextVNode, getCurrentInstance, h, inject, nextTick, provide, proxyRefs, ref, registerRuntimeCompile, renderSlots, toDisplayString };
